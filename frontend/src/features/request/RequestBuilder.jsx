...
//       let url = resolveUrl(path.startsWith('http') ? path : base + path)
...
//       // ── Smart routing ─────────────────────────────────────────
//       // localhost/127.0.0.1 → direct fetch (no proxy needed)
//       // external URLs       → route through backend proxy (avoids CORS)
//       const isExternal =
//         url.startsWith('http') &&
...
//         })
//       } else {
//         // Direct fetch for localhost APIs
//         res = await fetch(url, {
//           method: tab.method,
//           headers,
//           body,
...
//       // Also try to detect JSON even without content-type header
...
//         <span className="text-muted text-xs font-mono whitespace-nowrap">BASE URL</span>
//         <input
//           type="text"
//           value={tab.baseUrl ?? ''}
...
const RequestBuilder = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // After getting response, capture headers:
      ...
      dispatch(setTabResponse({
        ...
        response: {
          ...
          isJson,
          size: new Blob([rawText]).size,
          headers: responseHeaders, // Added response headers here
        },
      }))
  ...
  return (
    <div className="flex flex-col h-full bg-[#0f111a]">
      ...
      {/* BASE URL row */}
      <div className="flex items-center gap-3 px-4 pt-3 pb-2 shrink-0">
        <span className="text-slate-500 text-xs font-mono whitespace-nowrap uppercase tracking-wider">Base URL</span>
        <input
          type="text"
          value={tab.baseUrl ?? ''}
          onChange={(e) => dispatch(updateActiveTab({ baseUrl: e.target.value }))}
          placeholder="https://api.example.com"
          className="flex-1 bg-slate-800/40 border border-slate-800 rounded px-3 py-1.5 text-slate-100 text-xs font-mono outline-none focus:border-indigo-500 transition-colors"
        />
        {/* Routing indicator */}
        {tab.baseUrl && (
      ...
      {/* Method + URL + Save + Send */}
      <div className="flex items-center gap-2 px-4 pb-3 border-b border-slate-800 shrink-0">
        <select
          value={tab.method}
          onChange={(e) => dispatch(updateActiveTab({ method: e.target.value }))}
          className={`bg-[#1a1d2e] border border-slate-800 rounded px-3 py-2 text-sm font-mono font-bold outline-none cursor-pointer min-w-[80px] shrink-0 ${METHOD_COLORS[tab.method]}`}
        >
          {METHODS.map((m) => (
            <option key={m} value={m} className="text-slate-100 bg-[#1a1d2e]">{m}</option>
          ))}
        </select>
        ...
        <input
          ...
          className="flex-1 min-w-0 bg-transparent border border-slate-800 rounded px-4 py-2 text-slate-100 text-sm font-mono outline-none focus:border-indigo-500 transition-colors"
        />

        <button
          onClick={() => setShowSave(true)}
          disabled={!tab.url.trim()}
          className="border border-slate-800 hover:border-indigo-500 text-slate-500 hover:text-indigo-400 text-xs font-mono px-4 py-2 rounded transition-colors disabled:opacity-40 shrink-0"
        >
          {tab.id ? 'Update' : 'Save'}
        </button>
        ...
        <button
          onClick={handleSend}
          disabled={tab.isLoading || !tab.url.trim()}
          className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-sm font-mono font-bold px-6 py-2 rounded transition-colors flex items-center gap-2 shrink-0"
        >
          {tab.isLoading
            ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            : 'Send'
